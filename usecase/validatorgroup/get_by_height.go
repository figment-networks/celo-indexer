...
func (uc *getByHeightUseCase) Execute(height *int64) (SeqListView, error) {
	...
	validatorSeqs, err := uc.db.ValidatorGroupSeq.FindByHeight(*height)
	if len(validatorSeqs) == 0 || err != nil {
		...
		validatorSeqs = append(payload.NewValidatorGroupSequences, payload.UpdatedValidatorGroupSequences...)
	}
